      ...
      <div className="w-full flex items-center justify-between h-8 pwa-header-container">
        ...
        <div className="flex items-center h-full flex-shrink-0">
          ...
          <div className="flex items-center space-x-1 pr-1.5 sm:pr-2 pwa-no-drag">
            ...
            {/* Botón Acceso Directo a Recursos por Materia */}
            <button
              onClick={onOpenSubjectResourcesModal}
              title="Recursos, libros, guías y documentos por Materia y Tetramestre"
              className="inline-flex items-center justify-center space-x-1 h-[22px] px-2 rounded text-[10px] font-bold bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-200 hover:text-white border border-indigo-400/30 transition shadow-2xs active:scale-95 flex-shrink-0 cursor-pointer"
            >
              <BookOpen className="w-3 h-3 text-indigo-300" />
              <span className="hidden sm:inline">Recursos Materias</span>
            </button>

            {/* Botón Recursos Generales */}
            <button
                  ...
                  <>
                    ...
                    <div className="p-1 space-y-1">
                      <button
                        onClick={() => {
                          setShowUserMenu(false);
                          if (onOpenSubjectResourcesModal) onOpenSubjectResourcesModal();
                        }}
                        className="w-full flex items-center space-x-2 px-3 py-2 text-xs text-indigo-700 bg-indigo-50/70 hover:bg-indigo-100 rounded-xl transition font-bold text-left cursor-pointer"
                      >
                      ...
                      {onOpenConfigModal && (
                        <button
                          ...
                          className="w-full flex items-center space-x-2 px-3 py-2 text-xs text-slate-800 hover:bg-slate-100 rounded-xl transition font-semibold text-left cursor-pointer"
                        >
                          <Settings className={`w-3.5 h-3.5 ${isAdmin ? 'text-indigo-600' : 'text-slate-600'}`} />
                          <span>{isAdmin ? 'Configuración / Admin' : 'Configuración y Ajustes'}</span>
                        </button>
                      )}
