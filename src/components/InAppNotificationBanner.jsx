...
import { Bell, BellRing, X, ExternalLink, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const InAppNotificationBanner = () => {
  ...
  useEffect(() => {
    const handleNotification = (e) => {
      ...
      // Auto-eliminar a los 7 segundos
      setTimeout(() => {
  ...
  return (
    <div className="fixed top-4 inset-x-3 sm:inset-x-auto sm:right-4 z-[99999] flex flex-col space-y-2.5 max-w-sm w-full pointer-events-none mx-auto sm:mx-0">
      {notifications.map((notif) => (
        <div
          key={notif.id}
          className="pointer-events-auto bg-slate-900/98 backdrop-blur-xl text-white border-2 border-blue-500 shadow-2xl rounded-2xl p-4 flex items-start space-x-3.5 animate-in slide-in-from-top-6 fade-in duration-300 transition-all hover:border-blue-400"
        >
