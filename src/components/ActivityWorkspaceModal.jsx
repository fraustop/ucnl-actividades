...
export const ActivityWorkspaceModal = ({
  ...
  // SuscripciÃ³n en tiempo real a los recursos de la materia
  useEffect(() => {
  ...
  // SuscripciÃ³n a comentarios de actividad en tiempo real
  useEffect(() => {
  ...
  // Cargar automÃ¡ticamente el primer recurso al cambiar de actividad
  useEffect(() => {
  ...
  // Guardar programa / descripciÃ³n de la materia
  ...
  const handleDeleteSubjectAttachment = async (attachmentId) => {
    ...
    if (!window.confirm('Â¿Deseas eliminar este documento de la materia?')) return;
    try {
  ...
  const handleDeleteSubjectLink = async (linkId) => {
    ...
    if (!window.confirm('Â¿Deseas eliminar este enlace de la materia?')) return;
    try {
  ...
  // Manejar selecciÃ³n de una actividad desde el listado explorador
  ...
  const handleUploadFile = async (e) => {
    ...
    try {
      if (workspaceMode === 'subject') {
        ...
        setUploadSuccess(`Â¡Archivo "${uploadFile.name}" subido a ${currentSubject?.name || 'la materia'}!`);
        setUploadFile(null);
        if (newAttachment) {
        ...
        setUploadSuccess(`Â¡Archivo "${uploadFile.name}" subido exitosamente a la actividad!`);
        setUploadFile(null);
        if (newAttachment) {
          handleSelectResource({ url: newAttachment.downloadUrl, name: newAttachment.name });
        }
  ...
  const handleAddLink = async (e) => {
    ...
    try {
      ...
      if (workspaceMode === 'subject') {
        ...
        setLinkSuccess(`Â¡Enlace compartido en ${currentSubject?.name || 'la materia'}!`);
        ...
        setLinkSuccess('Â¡Enlace guardado y compartido en la actividad!');
        ...
        <div className="h-full flex flex-col items-center justify-center p-6 text-center text-slate-400 space-y-4 bg-slate-900/40">
          ...
          <div className="max-w-md space-y-1.5">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              {workspaceMode === 'subject'
                ? 'Haz clic en cualquier archivo, libro o enlace de la materia (panel izquierdo) para previsualizarlo aquÃ­ con soporte integrado.'
                : 'Haz clic en cualquier enlace o documento de las instrucciones (panel izquierdo) para previsualizarlo aquÃ­.'}
            </p>
          </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-slate-950 overflow-hidden relative">
        ...
        <div className="px-4 py-2.5 bg-slate-900 border-b border-slate-800 flex items-center justify-between gap-2 flex-shrink-0 text-white">
          ...
          <div className="flex items-center space-x-1.5 flex-shrink-0">
            ...
            <button
              ...
              title="Explicar por quÃ© algunos sitios rechazan la conexiÃ³n en la app"
            >
              <HelpCircle className="w-3.5 h-3.5 text-amber-400" />
              <span className="hidden sm:inline">Â¿Por quÃ© no abre?</span>
            </button>
            ...
            <button
              ...
              title="Abrir pÃ¡gina en el navegador predeterminado de tu equipo"
            >
        ...
        <div className="flex-1 w-full h-full relative flex items-center justify-center overflow-auto p-2 bg-slate-950">
          ...
          {type === 'video_direct' && (
            ...
            >
              Tu navegador no soporta la reproducciÃ³n directa de este formato de video.
            </video>
          )}
  ...
  return (
    ...
    >
      ...
      {/* Panel principal: siempre flex-row, nunca apilado */}
      <div
      ...
      >
        ...
        <div className="w-[320px] flex-shrink-0 flex flex-col h-full overflow-hidden bg-slate-50 border-r border-slate-200">
          ...
          <div className="px-3 py-2.5 bg-slate-900 border-b border-slate-800 flex-shrink-0">
            <div className="flex bg-slate-800 p-1 rounded-xl border border-slate-700/60 shadow-inner">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg text-xs font-bold transition cursor-pointer ${
              ...
              >
                ...
                {activities?.length > 0 && (
                  <span className="text-[9px] px-1.5 rounded-full bg-white/20 font-mono">{activities.length}</span>
                )}
              </button>
              <button
                type="button"
          ...
          {/* ---- MODO ACTIVIDAD: Explorador o Detalle de actividad ---- */}
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
              ...
              <div className="px-3.5 py-3 border-b border-slate-200 bg-white space-y-2.5 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-xs font-extrabold text-slate-900 flex items-center gap-1.5">
                        Actividades
                        <span className="text-[9px] font-bold px-1.5 rounded-full bg-blue-100 text-blue-800 border border-blue-200">
                ...
                <div className="relative">
                  ...
                  />
                  {explorerSearch && (
                    <button onClick={() => setExplorerSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 cursor-pointer">
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                  <select value={explorerTetra} onChange={(e) => setExplorerTetra(e.target.value)}
                    className="px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer">
                    <option value="all">Todos los Tetras</option>
                    {academicStructure.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </select>
                  <select value={explorerStatus} onChange={(e) => setExplorerStatus(e.target.value)}
                    className="px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer">
              ...
              <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5 min-h-0">
                {filteredExplorerActivities.length === 0 ? (
                  <div className="p-8 text-center text-slate-400 space-y-2">
                    ...
                    <p className="text-xs font-semibold">Sin actividades.</p>
                  </div>
                  ...
                  return (
                    <div key={act.id} onClick={() => handleSelectActivity(act)}
                      className="p-2.5 bg-white hover:bg-blue-50/70 rounded-xl border border-slate-200 hover:border-blue-300 transition cursor-pointer group space-y-1">
                      <div className="flex items-center justify-between gap-1">
                        <div className="flex items-center gap-1 flex-wrap min-w-0">
                          ...
                          <span className="text-[10px] font-bold text-blue-700 truncate max-w-[100px]">{act.subject}</span>
                        </div>
                        <span className={`px-1.5 rounded-full text-[9px] font-bold border ${dInfo.className} flex-shrink-0`}>{dInfo.text}</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-[10px] text-slate-500 pt-0.5 border-t border-slate-100">
                        ...
                        <span className={`font-bold ${isDone ? 'text-emerald-600' : inProg ? 'text-blue-600' : 'text-amber-600'}`}>
                          {isDone ? 'âœ…' : inProg ? 'ðŸš€' : 'â³'}
                        </span>
                      </div>
                    </div>
                  );
          ...
          {/* ---- MODO ACTIVIDAD: Detalle de actividad seleccionada ---- */}
          {workspaceMode === 'activity' && currentActivity && (
            <div className="flex-1 flex flex-col h-full overflow-hidden">
              {/* NavegaciÃ³n volver */}
              <div className="px-3 py-2 bg-slate-100 border-b border-slate-200 flex items-center justify-between gap-2 flex-shrink-0">
                <button
                  onClick={() => { setCurrentActivity(null); if (onSelectActivity) onSelectActivity(null); }}
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-xs font-bold text-blue-700 bg-white hover:bg-blue-50 border border-slate-200 transition shadow-2xs cursor-pointer active:scale-95"
                >
                  <ArrowLeft className="w-3.5 h-3.5" />
                  <span>Todas</span>
                </button>
                {activities?.length > 1 && (
                  <select value={currentActivity.id}
                    onChange={(e) => { const a = activities.find(x => x.id === e.target.value); if (a) handleSelectActivity(a); }}
                    className="max-w-[160px] text-[11px] font-bold px-2 py-1 bg-white border border-slate-200 rounded-xl text-slate-800 truncate focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-2xs cursor-pointer">
              ...
              <div className="px-3.5 py-3 border-b border-slate-200 bg-white space-y-1.5 flex-shrink-0">
                <div className="flex items-center justify-between gap-1.5">
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    <span className="px-2 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-800 border border-blue-200 truncate max-w-[140px]">{currentActivity.subject}</span>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${dueInfo?.className} flex-shrink-0`}>{dueInfo?.text}</span>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-3.5 space-y-4 min-h-0 text-sm text-slate-700">
                ...
                {directAction && (
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50/70 p-3 rounded-2xl border border-blue-200/90 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      <div className="p-1.5 rounded-xl bg-blue-600 text-white flex-shrink-0">
                        {directAction.platform === 'teams' || directAction.platform === 'meet' || directAction.platform === 'zoom' ? <Video className="w-3.5 h-3.5" /> : directAction.platform === 'whatsapp' ? <MessageCircle className="w-3.5 h-3.5" /> : <ExternalLink className="w-3.5 h-3.5" />}
                      </div>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={() => handleSelectResource({ url: directAction.url, title: directAction.label })} className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition flex items-center gap-1 cursor-pointer"><Eye className="w-3 h-3" /><span>Ver</span></button>
                ...
                <div className="space-y-1.5">
                  ...
                  {currentActivity.description ? (
                    <div className="bg-white p-3.5 rounded-2xl border border-slate-200 text-slate-800 leading-relaxed">
                      <RichTextRenderer content={currentActivity.description} onLinkClick={(url, label) => handleSelectResource({ url, title: label || url })} />
                    </div>
                  ) : (
              ...
              <div className="p-3 bg-white border-t border-slate-200 flex-shrink-0">
                <div className="flex items-center bg-slate-100 p-1 rounded-2xl gap-1">
                  {[
                    ...
                    <button key={status}
                      onClick={() => {
                        if (status === 'completed' && !isCompleted) { try { confetti({ particleCount: 60, spread: 70, origin: { y: 0.7 } }); } catch(e){} }
                        if (onSetPersonalStatus) onSetPersonalStatus(currentActivity.id, status);
                      }}
          ...
          {/* ---- MODO MATERIA: Filtro de Tetra + Lista de materias ---- */}
          {workspaceMode === 'subject' && (
            <div className="flex-1 flex flex-col h-full overflow-hidden">
              {/* Cabecera */}
              <div className="px-3.5 py-3 border-b border-slate-200 bg-white space-y-2.5 flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-xs font-extrabold text-slate-900 flex items-center gap-1.5">
                      Materias
                      <span className="text-[9px] font-bold px-1.5 rounded-full bg-indigo-100 text-indigo-800 border border-indigo-200">
                ...
                <div className="space-y-1">
                  ...
                  <select value={selectedTetraId} onChange={(e) => handleSubjectTetraChange(e.target.value)}
              ...
              <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5 min-h-0">
                {(currentTetra?.subjects || []).length === 0 ? (
                  <div className="p-8 text-center text-slate-400 space-y-2">
                    ...
                    <p className="text-xs font-semibold">Sin materias en este tetramestre.</p>
                  </div>
                ) : (currentTetra.subjects).map((sub) => {
                  const isSelected = sub.id === selectedSubjectId;
                  return (
                    <div key={sub.id} onClick={() => handleSubjectChange(sub.id)}
                      className={`p-2.5 rounded-xl border transition cursor-pointer group space-y-0.5 ${
                        isSelected
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className={`text-xs font-extrabold truncate ${isSelected ? 'text-indigo-800' : 'text-slate-900 group-hover:text-indigo-700'}`}>{sub.name}</span>
                        </div>
                        {isSelected && <span className="w-2 h-2 rounded-full bg-indigo-500 flex-shrink-0 animate-pulse" />}
                      </div>
        ...
        {/* ================================================================== */}
        {/* COLUMNA DERECHA â€” Panel oscuro unificado para ambos modos          */}
        {/* ================================================================== */}
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-900 text-slate-100 min-w-0">

          {/* Barra de tabs (misma estructura para ambos modos) */}
          <div className="px-3 py-2 bg-slate-950 border-b border-slate-800 flex items-center justify-between gap-2 overflow-x-auto no-scrollbar flex-shrink-0">
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              {workspaceMode === 'subject' && (
                <>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${activeTab === 'viewer' && !activeResource ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700'}`}
                  >
                  ...
                  {activeResource && (
                    <button onClick={() => setActiveTab('viewer')}
                      className="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer bg-blue-600 text-white shadow-sm">
                  ...
                  {canManageSubject && selectedSubjectId && (
                    <>
                      <button onClick={() => setActiveTab('upload')}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${activeTab === 'upload' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700'}`}>
                        <Upload className="w-3.5 h-3.5 text-emerald-400" /><span>Subir</span>
                      </button>
                      <button onClick={() => setActiveTab('share_link')}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${activeTab === 'share_link' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700'}`}>
                        <Link2 className="w-3.5 h-3.5 text-amber-400" /><span>Enlace</span>
                      </button>
                    </>
                  )}
                </>
              ...
              {workspaceMode === 'activity' && (
                <>
                  <button onClick={() => setActiveTab('viewer')}
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${activeTab === 'viewer' ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700'}`}>
                    <Film className="w-3.5 h-3.5 text-cyan-400" /><span>Visor</span>
                    {activeResource && <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />}
                  </button>
                  {currentActivity && (
                    <>
                      <button onClick={() => setActiveTab('comments')}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${activeTab === 'comments' ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700'}`}>
                        <MessagesSquare className="w-3.5 h-3.5 text-indigo-400" /><span>Comentarios</span>
                        {comments.length > 0 && <span className="px-1.5 rounded-full text-[10px] font-black bg-indigo-500 text-white">{comments.length}</span>}
                      </button>
                      <button onClick={() => setActiveTab('upload')}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${activeTab === 'upload' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700'}`}>
                        <Upload className="w-3.5 h-3.5 text-emerald-400" /><span>Subir</span>
            ...
            <button onClick={onClose}
              className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-slate-800 hover:bg-rose-600 text-slate-200 hover:text-white border border-slate-700 hover:border-rose-500 transition flex items-center gap-1.5 active:scale-95 cursor-pointer flex-shrink-0 ml-auto">
              <X className="w-3.5 h-3.5" /><span>Cerrar</span>
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            {/* VISOR (activo cuando hay un recurso seleccionado, en cualquier modo) */}
            {activeTab === 'viewer' && activeResource && renderIntegratedViewer()}

            {/* DETALLE DE MATERIA (subject mode, sin recurso activo) */}
                ...
                {!selectedSubjectId ? (
                  <div className="h-full flex flex-col items-center justify-center p-6 text-center text-slate-400 space-y-4">
                    ...
                    <div className="max-w-sm space-y-1.5">
                      ...
                      <p className="text-xs text-slate-400 leading-relaxed">Haz clic en cualquier materia del panel izquierdo.</p>
                    </div>
                  </div>
                ) : loadingSubject ? (
                  <div className="h-full flex flex-col items-center justify-center gap-3 text-slate-400">
                    <Loader2 className="w-8 h-8 animate-spin text-indigo-400" />
                    <p className="text-sm">Cargando {currentSubject?.name}...</p>
                  </div>
                ) : (
                  <>
                    {/* Header materia */}
                    <div className="flex items-center gap-3 pb-3 border-b border-slate-700">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-slate-400">
                          {currentSubject?.code && <span className="font-mono bg-slate-800 text-slate-300 px-1.5 py-0.5 rounded font-bold border border-slate-700">{currentSubject.code}</span>}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-indigo-400" />Programa / GuÃ­a Oficial</h3>
                        {canManageSubject && !isEditingSubjectDesc && (
                          <button type="button" onClick={() => setIsEditingSubjectDesc(true)}
                            className="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-bold text-indigo-300 bg-indigo-500/10 hover:bg-indigo-500/20 rounded-lg transition cursor-pointer border border-indigo-500/20">
                      ...
                      {isEditingSubjectDesc ? (
                        <div className="space-y-2 bg-slate-800 p-3 rounded-2xl border border-indigo-500/30">
                          <RichTextEditor value={subjectDescText} onChange={setSubjectDescText} placeholder="Programa, objetivos, criterios de evaluaciÃ³n..." />
                          <div className="flex items-center justify-end gap-2 pt-1">
                            <button type="button" onClick={() => { setIsEditingSubjectDesc(false); setSubjectDescText(subjectData?.description || ''); }}
                              className="px-3 py-1 rounded-xl text-xs font-bold text-slate-300 hover:bg-slate-700 cursor-pointer">Cancelar</button>
                            <button type="button" onClick={handleSaveSubjectDescription} disabled={isSavingSubjectDesc}
                              className="inline-flex items-center gap-1 px-3.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold disabled:opacity-50 cursor-pointer">
                              {isSavingSubjectDesc ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}<span>Guardar</span>
                            </button>
                          </div>
                        </div>
                      ) : subjectData?.description ? (
                        <div className="bg-slate-800/60 p-4 rounded-2xl border border-slate-700 text-slate-200 leading-relaxed">
                          <RichTextRenderer content={subjectData.description} onLinkClick={(url, label) => handleSelectResource({ url, title: label || url })} />
                        </div>
                      ) : (
                        <div className="p-4 rounded-2xl bg-slate-800/40 border border-slate-700 text-slate-500 text-xs italic">
                          {canManageSubject ? 'Sin programa. Haz clic en "Editar" para aÃ±adirlo.' : 'Sin programa oficial registrado.'}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5"><Paperclip className="w-3.5 h-3.5 text-indigo-400" />Documentos y Libros ({subjectData?.attachments?.length || 0})</h3>
                        {canManageSubject && <button type="button" onClick={() => setActiveTab('upload')} className="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-bold text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-lg transition cursor-pointer border border-emerald-500/20"><Plus className="w-3 h-3" /><span>Subir</span></button>}
                      </div>
                      {(!subjectData?.attachments || subjectData.attachments.length === 0) ? (
                        <div className="p-3 rounded-2xl bg-slate-800/40 border border-slate-700 text-slate-500 text-xs italic">No hay documentos en esta materia.</div>
                      ) : (
                        <div className="space-y-1.5">
                          {subjectData.attachments.map((file) => (
                            <div key={file.id} className="flex items-center justify-between p-2.5 rounded-xl border border-slate-700 bg-slate-800/60 hover:border-indigo-500/50 transition">
                              <div className="flex items-center gap-2 truncate pr-2"><FileIcon category={file.category} className="w-4 h-4 flex-shrink-0" /><div className="truncate"><p className="font-semibold text-slate-200 text-xs truncate">{file.name}</p><p className="text-[10px] text-slate-500">{formatBytes(file.size)}{file.uploadedBy && ` â€¢ ${file.uploadedBy}`}</p></div></div>
                              <div className="flex items-center gap-1 flex-shrink-0">
                                <button onClick={() => handleSelectResource({ url: file.downloadUrl, name: file.name })} className="px-2.5 py-1 text-xs font-bold text-indigo-300 bg-indigo-500/10 hover:bg-indigo-600 hover:text-white rounded-lg transition flex items-center gap-1 cursor-pointer"><Eye className="w-3 h-3" /><span>Ver</span></button>
                                <a href={file.downloadUrl} target="_blank" rel="noopener noreferrer" download={file.name} className="p-1 text-slate-500 hover:text-blue-400 rounded-lg transition"><Download className="w-3.5 h-3.5" /></a>
                                {canManageSubject && <button type="button" onClick={() => handleDeleteSubjectAttachment(file.id)} className="p-1 text-slate-500 hover:text-rose-400 rounded-lg transition cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>}
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <h3 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5"><ExternalLink className="w-3.5 h-3.5 text-indigo-400" />Enlaces y Videos ({subjectData?.links?.length || 0})</h3>
                        {canManageSubject && <button type="button" onClick={() => setActiveTab('share_link')} className="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-bold text-blue-300 bg-blue-500/10 hover:bg-blue-500/20 rounded-lg transition cursor-pointer border border-blue-500/20"><Plus className="w-3 h-3" /><span>AÃ±adir</span></button>}
                      </div>
                      {(!subjectData?.links || subjectData.links.length === 0) ? (
                        <div className="p-3 rounded-2xl bg-slate-800/40 border border-slate-700 text-slate-500 text-xs italic">No hay enlaces en esta materia.</div>
                      ) : (
                        <div className="space-y-1.5">
                          {subjectData.links.map((link) => (
                            <div key={link.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700 hover:border-indigo-500/50 transition">
                              <div className="flex items-center gap-2 truncate pr-2"><ExternalLink className="w-3.5 h-3.5 text-indigo-400 flex-shrink-0" /><span className="font-semibold text-xs text-slate-200 truncate">{link.title || link.url}</span></div>
                              <div className="flex items-center gap-1 flex-shrink-0">
                                <button onClick={() => handleSelectResource(link)} className="px-2.5 py-1 text-xs font-bold text-indigo-300 bg-indigo-500/10 hover:bg-indigo-600 hover:text-white rounded-lg transition flex items-center gap-1 cursor-pointer"><Eye className="w-3 h-3" /><span>Visor</span></button>
                                <a href={link.url} target="_blank" rel="noopener noreferrer" className="p-1 text-slate-500 hover:text-blue-400 rounded-lg transition"><ExternalLink className="w-3 h-3" /></a>
                                {canManageSubject && <button type="button" onClick={() => handleDeleteSubjectLink(link.id)} className="p-1 text-slate-500 hover:text-rose-400 rounded-lg transition cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>}
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            {/* VISOR VACÃO (activity mode, sin recurso activo) */}
            {workspaceMode === 'activity' && activeTab === 'viewer' && !activeResource && renderIntegratedViewer()}

            {/* COMENTARIOS (activity mode) */}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3.5 min-h-0">
                  {comments.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center p-6 text-center text-slate-400 space-y-3">
                      ...
                      <div><p className="font-bold text-slate-200 text-sm">Sin comentarios aÃºn</p><p className="text-xs text-slate-400 mt-1">SÃ© el primero en hacer una pregunta o dejar retroalimentaciÃ³n.</p></div>
                    </div>
                  ) : comments.map((cmt) => {
                    ...
                    return (
                      <div key={cmt.id} className={`p-3.5 rounded-2xl border transition ${isAuthor ? 'bg-indigo-950/40 border-indigo-800/60 ml-4 sm:ml-8' : 'bg-slate-800/80 border-slate-700 mr-4 sm:mr-8'}`}>
                        <div className="flex items-center justify-between gap-2 mb-1.5">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-[10px] uppercase flex-shrink-0">{cmt.userName ? cmt.userName[0] : 'U'}</div>
                            <span className="font-bold text-xs text-white truncate">{cmt.userName}</span>
                            <span className={`text-[9px] font-bold px-1.5 py-0.2 rounded uppercase ${cmt.userRole === 'admin' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : cmt.userRole === 'docente' ? 'bg-blue-500/20 text-blue-300 border border-blue-500/30' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'}`}>{cmt.userRole}</span>
                          </div>
                        ...
                        {cmt.links?.length > 0 && (
                          <div className="mt-2 space-y-1">
                            {cmt.links.map((l, i) => (
                              <button key={i} onClick={() => handleSelectResource(l)} className="flex items-center gap-1.5 text-xs text-cyan-300 hover:underline bg-black/30 px-2.5 py-1 rounded-lg border border-white/10 cursor-pointer">
                                <ExternalLink className="w-3 h-3" /><span className="truncate">{l.title || l.url}</span><span className="text-[10px] text-slate-400">(Visor)</span>
                              </button>
                            ))}
                          </div>
                        )}
                ...
                <form onSubmit={handleSendComment} className="p-3.5 bg-slate-950 border-t border-slate-800 space-y-2 flex-shrink-0">
                  <div className="flex items-center justify-between text-[11px] px-1 text-slate-400">
                    <div className="flex items-center gap-1.5">
                      <span>Tu hilo:</span>
                      <span className={`font-mono font-bold ${userThreadBytes > 950000 ? 'text-rose-400' : userThreadBytes > 800000 ? 'text-amber-400' : 'text-slate-300'}`}>{formatBytes(userThreadBytes)} / 1 MB</span>
                    </div>
                    {userThreadBytes > 800000 && <span className="text-amber-400 text-[10px]">âš ï¸ Cerca del lÃ­mite</span>}
                  </div>
                  {commentError && <div className="p-2 bg-rose-950 border border-rose-800 rounded-xl text-rose-200 text-xs">{commentError}</div>}
                  <div className="flex gap-2">
                    <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder="Escribe una duda, apunte o respuesta..." rows={2}
                    ...
                    <button type="submit" disabled={sendingComment || (!commentText.trim() && !commentLinkInput.trim())}
                      className="px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:opacity-40 text-white rounded-2xl font-bold text-xs flex items-center justify-center gap-1.5 transition flex-shrink-0 cursor-pointer">
                  ...
                  <input type="url" value={commentLinkInput} onChange={(e) => setCommentLinkInput(e.target.value)} placeholder="Adjuntar URL (opcional): https://..."
                    className="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-[11px] text-slate-300 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                </form>
              </div>
            )}

            {/* SUBIR DOCUMENTOS (ambos modos) */}
            {activeTab === 'upload' && (
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                <div className="p-4 bg-slate-800/80 rounded-2xl border border-slate-700 space-y-1">
                  ...
                  <p className="text-xs text-slate-400">{workspaceMode === 'subject' ? 'Libros, manuales, guÃ­as o diapositivas para esta materia.' : 'PDF, Word, Excel, PowerPoint, imÃ¡genes o videos.'}</p>
                </div>
                ...
                <form onSubmit={handleUploadFile} className="space-y-4">
                  <div className="border-2 border-dashed border-slate-700 hover:border-blue-500 rounded-3xl p-6 text-center space-y-3 bg-slate-900/60 transition">
                    ...
                    <div><p className="text-xs font-bold text-white">Selecciona un archivo</p><p className="text-[11px] text-slate-400 mt-0.5">PDF, DOCX, XLSX, PPTX, JPG, PNG, MP4, ZIP (mÃ¡x 50MB)</p></div>
                    ...
                    {uploadFile && (
                      <div className="p-2.5 bg-slate-800 rounded-xl border border-slate-700 text-xs text-slate-200 flex items-center justify-between">
                        <div className="flex items-center gap-2 truncate"><FileIcon category={uploadFile.name.split('.').pop()} className="w-4 h-4" /><span className="truncate font-semibold">{uploadFile.name}</span></div>
                  ...
                  {isUploading && (
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-xs text-slate-300 font-bold"><span>Subiendo...</span><span>{uploadProgress}%</span></div>
                      <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 transition-all duration-200" style={{ width: `${uploadProgress}%` }} /></div>
                    </div>
                  )}
                  <button type="submit" disabled={isUploading || !uploadFile}
                    className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition active:scale-95 cursor-pointer">
            ...
            {/* COMPARTIR ENLACE (ambos modos) */}
            {activeTab === 'share_link' && (
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                <div className="p-4 bg-slate-800/80 rounded-2xl border border-slate-700 space-y-1">
                  <h4 className="text-sm font-bold text-white flex items-center gap-2"><Link2 className="w-4 h-4 text-amber-400" /><span>{workspaceMode === 'subject' ? `AÃ±adir Enlace a ${currentSubject?.name || 'la Materia'}` : 'Compartir Enlace o Video'}</span></h4>
                  <p className="text-xs text-slate-400">YouTube, Google Drive, Loom, Vimeo, OneDrive o pÃ¡ginas web.</p>
                </div>
                ...
                <form onSubmit={handleAddLink} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="url" required value={newLinkUrl} onChange={(e) => setNewLinkUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..."
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-300">TÃ­tulo (opcional)</label>
                    <input type="text" value={newLinkTitle} onChange={(e) => setNewLinkTitle(e.target.value)} placeholder="Ej. Clase grabada del tema 3..."
                      className="w-full p-3 rounded-2xl bg-slate-800 border border-slate-700 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <button type="submit" disabled={savingLink || !newLinkUrl.trim()}
                    className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition active:scale-95 cursor-pointer">
          ...
          </div>{/* fin contenido columna derecha */}
        </div>{/* fin columna derecha */}
      </div>{/* fin panel */}

      {/* Modal explicaciÃ³n X-Frame-Options */}
      {showSecurityExplanation && (
        <div className="absolute inset-0 z-10 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 max-w-lg w-full text-white shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-amber-400"><ShieldAlert className="w-5 h-5" /><h3 className="font-bold text-sm">Â¿Por quÃ© algunos sitios no abren dentro de la app?</h3></div>
              <button onClick={() => setShowSecurityExplanation(false)} className="p-1 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800"><X className="w-4 h-4" /></button>
            </div>
            <p className="text-xs text-slate-300 leading-relaxed">Por polÃ­ticas de seguridad del navegador (<code className="text-amber-300 bg-slate-800 px-1 py-0.5 rounded">X-Frame-Options: SAMEORIGIN</code>), algunos sitios rechazan ser incrustados para proteger tus contraseÃ±as contra ataques de clickjacking.</p>
            <div className="p-3 bg-blue-950/40 border border-blue-800/60 rounded-2xl space-y-2 text-xs text-blue-200">
              <p className="font-bold flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5 text-blue-400" />Soluciones integradas en UCNL:</p>
              <ul className="list-disc list-inside space-y-1 text-slate-300 text-[11px]">
                <li><strong className="text-white">Abrir en Navegador:</strong> Abre en Chrome, Edge o Safari.</li>
                <li><strong className="text-white">Ventana Paralela:</strong> Ventana flotante al lado de la app.</li>
                <li><strong className="text-white">Google Docs Viewer:</strong> Para PDFs y Office bloqueados.</li>
              </ul>
            </div>
